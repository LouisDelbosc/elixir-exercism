defmodule ScaleGenerator do
  @scale ~w(C C# D D# E F F# G G# A A# B C)
  ...
  def step(scale, tonic, "m"), do: get_tonic(scale, tonic, 1)
  def step(scale, tonic, "M"), do: get_tonic(scale, tonic, 2)
  def step(scale, tonic, "A"), do: get_tonic(scale, tonic, 3)

  def get_tonic(scale, tonic, step) do
    index = Enum.find_index(scale, fn tone -> tone == tonic end)
    Enum.at(scale, rem(index + step, length(scale)))
  end
  ...
  def chromatic_scale(tonic \\ "C") do
    index = Enum.find_index(@scale, fn tone -> tone == String.upcase tonic end)
    Enum.slice(@scale, index..-1) ++ Enum.slice(@scale, 1..index)
  end
  ...
  def flat_chromatic_scale(tonic \\ "C") do
    index = Enum.find_index(@flat_scale, fn tone -> tone == tonic end)
    Enum.slice(@flat_scale, index..-1) ++ Enum.slice(@flat_scale, 1..index)
  end
  ...
  def find_chromatic_scale(tonic) when tonic in ~w(F Bb Eb Ab Db Gb d g c f bb eb) do
    tonic |> String.capitalize |> flat_chromatic_scale
  end
  ...
  def scale(tonic, pattern) do
    ...
    pattern
    |> String.graphemes
    |> Enum.reduce([String.capitalize(tonic)], fn(step, [ h | t ]) ->
      [ step(scale, h, step) | [ h | t ] ]
    end)
    |> Enum.reverse
  end
end
